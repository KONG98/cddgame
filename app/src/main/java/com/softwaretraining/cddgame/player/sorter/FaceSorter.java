...
public class FaceSorter implements IConcreteSorter {

    private final Sorter sorter;

    FaceSorter(Sorter sorter) {
        this.sorter = sorter;
    }
    ...
    public int[] sort(int[] cardsArray) {
        ...
        for (int i = 0; i < cardsArray.length; i++) {
            int lowIndex = i;
            for (int j = cardsArray.length - 1; j > i; j--) {
                if (id2card(cards[j]).compareTo(id2card(cards[lowIndex])) > 0) {
                    lowIndex = j;
                }
            }
    ...
    public void nextSorter() {
        sorter.setConcreteSorter(sorter.getSuitSorter());
    }
